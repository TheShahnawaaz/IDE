...
main();`;
  ...
  // Reference for throttling drag operations
  const dragThrottleTimeout = useRef(null);
  ...
  const handleOutputDragMove = useCallback((e) => {
    if (isDraggingOutput.current) {
      // Use a timeout to throttle updates
      if (!dragThrottleTimeout.current) {
        dragThrottleTimeout.current = setTimeout(() => {
          dragThrottleTimeout.current = null;
        
          if (isHorizontalLayout) {
            // For horizontal layout
            const deltaY = dragStartY.current - e.clientY;
            // Allow resizing up to 70% of the screen height
            ...
            const containerWidth = window.innerWidth - (isExplorerVisible ? explorerWidth : 0) - 10;
            const newPercent = Math.max(
              10, // Min percent
              Math.min(50, // Max percent
                dragStartHeight.current + (deltaX / containerWidth) * 100)
            );
            setOutputHeight(newPercent);
          }
        }, 10); // 10ms throttle
      }
    }
